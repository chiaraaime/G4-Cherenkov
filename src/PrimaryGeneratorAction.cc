...
#include "G4ios.hh"
#include "G4ParticleGun.hh" //Chiara
...
{
  ///---------------------------------------
  //Chiara: for gun particle
  G4int nofParticles = 1; //number of particle per event
  ...
  fParticleGun->SetParticleMomentumDirection(G4ThreeVector(0.,0.,1.)); // è solo la direzione non la quantità di moto
...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* event)
{
  ...
  G4ThreeVector momentum(0, 0, 1.*MeV);
  G4double time = 0;
  ...
  fParticleGun->SetParticlePosition(G4ThreeVector(0., 0., -15.*cm)); //Chiara
  fParticleGun->GeneratePrimaryVertex(event); //Chiara
}
